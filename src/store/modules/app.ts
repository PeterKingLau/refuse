...
const defaultTheme: ThemeTypes = {
  ...
  // 左侧菜单选中背景颜色
  leftMenuBgActiveColor: 'var(--app-color-primary)',
  // 左侧菜单收起选中背景颜色
  leftMenuCollapseBgActiveColor: 'var(--app-color-primary)',
  // 左侧菜单字体颜色
...
const getCachedTheme = () => {
  const cachedTheme = wsCache.get('theme') || {}
  const { [legacyPrimaryKey]: legacyPrimary, ...restTheme } = cachedTheme
...
export const useAppStore = defineStore('app', {
  state: (): AppState => {
    return {
      userInfo: 'userInfo', // 登录信息存储字段，建议每个项目替换一个字段，避免与其他项目冲突
      ...
      uniqueOpened: false, // 是否只保持一个子菜单展开
      ...
      footer: true, // 显示页脚
      greyMode: false, // 是否开启灰色模式，用于特殊纪念日
      dynamicRouter: wsCache.get('dynamicRouter') || false, // 是否动态路由
      fixedMenu: wsCache.get('fixedMenu') || false, // 是否固定菜单

      layout: wsCache.get('layout') || 'classic', // layout甯冨眬
