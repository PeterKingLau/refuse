  ...
  <Form :schema="schema" :rules="rules" label-position="top" hide-required-asterisk size="large" class="dark:(border-1 border-[var(--app-border-color)] border-solid)" @register="register">
    ...
    <template #tool>
      <div class="flex justify-between items-center w-[100%]">
        <ACheckbox v-model:checked="remember">{{ t('login.remember') }}</ACheckbox>
        <AButton type="link" class="p-0">{{ t('login.forgetPassword') }}</AButton>
      </div>
    </template>

    <template #login>
      <div class="w-[100%]">
        <AButton :loading="loading" type="primary" class="w-[100%]" @click="signIn">
          {{ t('login.login') }}
        </AButton>
      </div>
      <div class="w-[100%] mt-15px" style="display: none">
        <AButton class="w-[100%]" @click="toRegister">
          {{ t('login.register') }}
        </AButton>
      </div>
    </template>
